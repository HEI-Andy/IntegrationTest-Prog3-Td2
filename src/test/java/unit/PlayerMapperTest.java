...
import static utils.TestUtils.nonExistingTeam;
import static utils.TestUtils.playerEntityRakoto;
import static utils.TestUtils.playerModelRakoto;
import static utils.TestUtils.playerModelWith_aNonExistingTeam;
...
public class PlayerMapperTest {
    ...
    private static PlayerEntity entityJeremy() {
        return playerEntityRakoto(teamBarea());
    }


    private static PlayerScorer jeremyScorer() {
        return rakotoModelScorer(
                playerModelRakoto(entityJeremy()),
                scorerRakoto(playerEntityRakoto(teamBarea())));
    }

    @Test
    void player_to_domain_ok() {
        PlayerEntity entity = entityJeremy();
    ...
    void player_scorer_to_domain_ok() {
        ...
        assertEquals(jeremyScorer(), actual);
    }
    ...
    @Test
    void player_ok() {
        when(teamRepositoryMock.findByName(teamBarea().getName())).thenReturn(teamBarea());

        PlayerEntity expected = playerEntityRakoto(teamBarea());
        PlayerEntity actual = subject.toEntity(
                playerModelRakoto(expected)
        );
    ...
    void player_to_entity_ko() {
        when(teamRepositoryMock.findByName(nonExistingTeam().getName())).thenReturn(null);

        String expectedMessage = new StringBuilder(HttpStatus.BAD_REQUEST.toString())
                .append(" : ")
                .append("Team with name ")
                .append(nonExistingTeam().getName())
                .append("not found.")
                .toString();
        ...
        assertThrowsExceptionMessage(
                ...
                () -> subject.toEntity(playerModelWith_aNonExistingTeam())
        );
    }
}
